...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not approved(user.id):
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("🛡️ Request access", callback_data="request_access")]])
        ...
        return
    await update.message.reply_text('🤖 <b>Telegram VPS Runner</b>\n\n📁 Send a <code>.py</code>, <code>.js</code>, or <code>.zip</code> file to create a project.\n\n Use /help for available commands.', parse_mode=ParseMode.HTML, reply_markup=menu())
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text('📨 File received!\n\nProject name ကို နောက်တစ်ကြောင်းမှာ ရိုးရိုးပို့ပါ။')


async def plain_project_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not guard(update) or not update.message or not update.message.text:
        return
    if not context.user_data.get("pending_upload"):
        return
...
def main() -> None:
    ...
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, plain_project_name))
